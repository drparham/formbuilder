...
@else
    <div class="form-group @if($errors->has($field) ) has-error @endif">
        ...
        <div class="col-md-9">
            <input type="date" class="form-control" id="{{$field}}" name="{{$field}}" <?php if($required=="NO") echo 'required';?> placeholder="{{ trans($trans.'model.general.'.$field) }}" value="<?php if(isset($fieldData)){ echo $fieldData; }?>">
            <span class="help-block">{{$errors->first($field)}}</span>
        </div>
    </div>
@endif